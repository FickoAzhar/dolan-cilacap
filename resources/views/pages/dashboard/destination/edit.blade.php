...
@section('content')
    <div class="p-4">
        <form id="fileUploadForm" method="POST" action="{{ url('dashboard/destinations/' . $destination->id) }}"
            enctype="multipart/form-data" class="md:w-2/3 w-full">
            @method('put')
            @csrf

            {{-- gambar --}}
            @if ($destination->image)
                <img src="{{ asset('media/' . $destination->image) }}" class="h-15 aspect-[4/3] img-preview block mb-3">
            @else
            ...
            <input id="name" type="text" name="name"
                class="w-full py-1 px-2 md:py-2 md:px-4 border-2 rounded-sm mb-2"
            ...
            <textarea rows="9" cols="70" id="deskripsi" type="text" name="deskripsi"
                class="w-full py-1 px-2 md:py-2 md:px-4 border-2 rounded-sm mb-2"
                value="{{ old('deskripsi', $destination->deskripsi) }}" required> {{ old('deskripsi', $destination->deskripsi) }}
            </textarea>
            @error('deskripsi')
                <p class="text-sm text-red-600">{{ $message }}</p>
            @enderror
            ...
            <input id="video" type="text" name="video"
                class="w-full py-1 px-2 md:py-2 md:px-4 border-2 rounded-sm mb-2"
                value="{{ old('video', $destination->video) }}" required>
            @error('video')
                <p class="text-sm text-red-600">{{ $message }}</p>
            @enderror
            ...
            <input id="harga" type="text" name="harga"
                class="w-full py-1 px-2 md:py-2 md:px-4 border-2 rounded-sm mb-2"
                value="{{ old('harga', $destination->harga) }}" required>
            @error('harga')
                <p class="text-sm text-red-600">{{ $message }}</p>
            @enderror
            ...
            <div class="flex justify-center mt-2">
                <button type="submit"
                    class="w-full text-sm md:text-base py-2 px-2 md:py-2 md:px-3 bg-red-500 text-white rounded-md shadow-sm mb-2 flex gap-1 justify-center items-center hover:bg-red-400">Ubah</button>
            </div>
        </form>
    </div>
@endsection
