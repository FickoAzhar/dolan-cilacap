...
@section('content')
    <div class="flex-1 px-2 md:px-3 mt-14  md:mt-0 min-h-screen pt-2">
        ...
        <div class="w-full flex justify-end">
            <a href="{{ url('dashboard/destinations/create') }}"
            ...
            <table class="table-auto w-full">
                <thead class="border-b-2 border-gray-200 bg-white">
                    <tr>
                        ...
                        <th class="p-3 text-sm font-semibold tracking-wide text-left text-black">harga</th>
                ...
                <tbody>
                    @foreach ($destinations as $destination)
                        <tr class="{{ $loop->index % 2 == 1 ? 'bg-white' : '' }}">
                            <td class="p-3 ">
                                <img src="{{ asset('media/' . $destination->image) }}" alt="{{ $destination->name }}"
                                    class="h-24 aspect-[4/3]">
                            </td>
                            <td class="p-3 text-sm text-gray-700">{{ $destination->name }}</td>
                            <td class="p-3 text-sm text-gray-700"><article>{!! Str::limit($destination->deskripsi, 50) !!}</article></td>
                            
                            <td class="p-3 text-sm text-gray-700">{{ $destination->harga }}</td>
                            <td class="p-3 text-sm text-gray-700">
                                <div class="flex flex-row gap-2">
                                    <a href="{{ url('dashboard/destinations/' . $destination->id . '/edit') }}">
                                        <i class="fa-solid fa-pen-to-square fa-lg hover:text-red-500"></i>
                                    </a>
                                    <form action="{{ url('dashboard/destinations/' . $destination->id) }}" method="POST">
                                        @method('delete')
                                        @csrf
